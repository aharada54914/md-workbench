...
const showcaseSource = readFileSync('docs/math-showcase.md', 'utf8');

for (const [name, newline] of [['LF', '\n'], ['CRLF', '\r\n']]) {
  const showcase = showcaseSource.replace(/\r\n|\r|\n/g, newline);
  test(`math showcase ${name}: edit, save, reopen, offline print and Marp`, async ({ page, context }, testInfo) => {
