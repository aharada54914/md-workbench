...
for (const operation of ['clean reload', 'dirty reload', 'save conflict'] as const) {
  ...
  test(`${operation} targets the active duplicate and its conflict choice`, async ({ page }) => {
    let releaseWatchReads!: () => void;
    const watchReadGate = new Promise<void>(resolve => { releaseWatchReads = resolve; });
    await setupTauriMocks(page, {
      initialFs: { [a]: source, [b]: source }, openFilePaths: [a, b],
      beforeWatchRead: () => watchReadGate,
    });
    try {
      await page.goto('/');
      await expect(page.locator('.tab-bar .tab')).toHaveCount(2);
      ...
      // Polling observes writes without events. Hold its completion explicitly so
      // this case tests only the manual/save operation's captured tab identity.
