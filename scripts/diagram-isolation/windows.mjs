...
try {
 ...
 for(const [context,target] of [['wrapper',fixture],['child',frame]]){
  report.checks[`${context}Bridge`]=await target.evaluate(key=>{
   const bridge=window.chrome?.webview;if(!bridge)return 'unavailable';
   for(const cmd of ['native_read_path','native_get_grant','create_new_window','ai_send'])bridge.postMessage(JSON.stringify({cmd,payload:{path:'fixture-unselected',windowLabel:'main'},callback:1,error:2,options:{customProtocolIpcBlocked:true},__TAURI_INVOKE_KEY__:key}));return 'sent';
  },key);
  assert.equal(report.checks[`${context}Bridge`],'sent','Transport unavailable is not proof of native denial');
 }
 const counts=()=>{const result={};for(const line of stderr.split('\n')){if(!line.startsWith('MDW_DIAGRAM_IPC_DENIED '))continue;try{const {command}=JSON.parse(line.slice('MDW_DIAGRAM_IPC_DENIED '.length));result[command]=(result[command]??0)+1}catch{/* Incomplete stream line. */}}return result};
 await until(()=>['native_read_path','native_get_grant','create_new_window','ai_send'].every(cmd=>counts()[cmd]===2),5000);
 report.checks.nativeCustomDeniedReceipts=counts();
 ...
 report.status='passed_measured_subset';
 }
